    ...
    for line in lines:
        if line.startswith("latitude"):
            ...
            if match.group("direction") == "N":
                ...
                assert (
                    match.group("direction") == "S"
                ), "Direction is not 'N' or 'S'! Check input"
                output_dict["latitude"] = -float(match.group("value"))
        ...
        if line.startswith("longitude"):
            ...
            if match.group("direction") == "E":
                ...
                assert (
                    match.group("direction") == "W"
                ), "Direction is not 'W' or 'E'! Check input"
                output_dict["longitude"] = -float(match.group("value"))

        if line.startswith("time"):
            output_dict["time_stamp"] = pd.to_datetime(
                line[5:-1], format="%Y %m %d %H %M %S"
            )
        ...
        if line.startswith("output_process"):
            output_dict["integrate_flag"] = (
                True if line[15:].strip() == "integrate" else False
            )
